...
def gen_eta21_pt60_conv(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    conv = events.FitChi2 < 10000
    return events, ak.all([eta, pt, conv], axis=0)
...
def gen_eta21_pt60_nconv(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_corr_deltaRmin08_t1(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_corr_deltaRmin06_t1(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_corr_deltaR06_q1(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaR06_q1(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaR04_06_q1(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRrecoJet08_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_recoJet_q1q2 < 0.8
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRrecoJet06_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_recoJet_q1q2 < 0.6
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRrecoJet04_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_recoJet_q1q2 < 0.4
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRgen08_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_inf_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_08_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = (0.6 < events.deltaR_gen_q1q2) & (events.deltaR_gen_q1q2 < 0.8)
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRgen04_06_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_inf_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_08_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_06_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_q1q2_corr_pgof(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2_corr_pgof(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_q1q2_corr_pgof(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def deltaRgen08_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    dR = events.deltaR_gen_q1q2 < 0.8
    return events, ak.all([dR], axis=0)
...
def deltaRgen06_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    dR = events.deltaR_gen_q1q2 < 0.6
    return events, ak.all([dR], axis=0)
...
def deltaRgen04_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    dR = events.deltaR_gen_q1q2 < 0.4
    return events, ak.all([dR], axis=0)
...
@categorizer(uses={"gen_top.*",
                   "fitCombinationType",
                   "dRmin_gen_t1",
                   "deltaR_gen_b_q1q2_min",
                   })
def gen_cut_deltaRminbq1q2_08_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_gen_b_q1q2_min < 0.8
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRminbq1q2_06_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_gen_b_q1q2_min < 0.6
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRminbq1q2_04_q1q2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_gen_b_q1q2_min < 0.4
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRminbq1q2_08_q1q2_corrB(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRminbq1q2_06_q1q2_corrB(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRminbq1q2_04_q1q2_corrB(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRminbq1q2_08_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRminbq1q2_06_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRminbq1q2_04_q1q2_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_q1q2_closest(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2_closest(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
    ...
    dR = events.deltaR_q1q2_closest < 0.6
    return events, ak.all([eta, pt, dR], axis=0)
...
def gen_cut_deltaRgen04_q1q2_closest(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen08_q1q2_closest_conv(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2_closest_conv(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_q1q2_closest_conv(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen06_q1q2_closest_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
...
def gen_cut_deltaRgen04_q1q2_closest_corr(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    pt_cut = 60
    eta = ak.all([
        ak.all(abs(events.gen_top.b.eta) < 2.1, axis=1),
        ak.all(abs(events.gen_top.w_children[:, :, 0].eta) < 2.1, axis=1),
