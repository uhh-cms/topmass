...
def aj_trighist_fill_hist(self: HistProducer, h: hist.Histogram, data: dict[str, Any], task: law.Task) -> None:
    ...
    if convert:
        ...
        weights = data["weight"]
        data["weight"] = weights[0]
        data2["weight"] = weights[1]
...
def trig_all_weights(self: HistProducer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    weight2 = ak.Array(np.ones(len(events)))
...
def bkg_all_weights(self: HistProducer, events: ak.Array, **kwargs) -> ak.Array:
    """
    HistProducer that combines all event weights from the *event_weights* aux entry from either the config or the
    dataset. The weights are multiplied together to form the full event weight.
    ...
    if self.dataset_inst.is_data and len(events):
        if has_ak_column(events, "bkg_weight"):
            ...
            self.logger.warning_once(
                "missing_dataset_weight_bkg_weight",
                f"weight 'bkg_weight' for dataset {self.dataset_inst.name} not found",
            )

    return events, weight


@bkg_all_weights.init
def bkg_all_weights_init(self: HistProducer) -> None:
    weight_columns = set()
