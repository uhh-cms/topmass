...
def no_trig_weight(self: WeightProducer, events: ak.Array, **kwargs) -> ak.Array:
    """
    ...
    if self.dataset_inst.is_mc and len(events):
        ...
        weight_list = self.config_inst.x.event_weights
        weight_list.pop("trig_weight")
        for column in weight_list:
...
@no_trig_weight.init
def no_trig_weight_init(self: WeightProducer) -> None:
