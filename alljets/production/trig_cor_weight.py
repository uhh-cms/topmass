...
@producer(
    ...
    mc_only=True,
    # function to determine the correction file
    get_trig_file=(lambda self, external_files: external_files.trig_sf_ht_after_pt_after_ht),
    # function to determine the trigger weight config
    # get_trig_config=(lambda self: self.config_inst.x.trig_sf_names),
)
def trig_weights_ht_after_pt_after_ht(
    self: Producer,
    events: ak.Array,
    # trig_mask: ak.Array | type(Ellipsis) = Ellipsis,
    **kwargs,
    ...
    if self.dataset_inst.has_tag("has_top"):
        ht = ak.sum(events.Jet.pt[(events.Jet.pt > 30) & (abs(events.Jet.eta) < 2.6)], axis=1)
        weight = self.trig_sf_corrector(ht, 40.0)
        events = set_ak_column(events, "trig_weight_3", weight, value_type=np.float32)
    else:
        events = set_ak_column(events, "trig_weight_3", np.ones(len(events)), value_type=np.float32)
    return events


@trig_weights_ht_after_pt_after_ht.requires
def trig_weights_ht_after_pt_after_ht_requires(self: Producer, reqs: dict) -> None:
    if "external_files" in reqs:
        return
...
@trig_weights_ht_after_pt_after_ht.setup
def trig_weights_ht_after_pt_after_ht_setup(
    self: Producer,
    reqs: dict,
    inputs: dict,
    reader_targets: InsertableDict,
