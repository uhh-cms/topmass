...
setup_topmass() {
    ...
    #   - "TM_", for features provided by the top mass analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "TM_" variables is documented below.
    #
    ...
    if [ "${TM_SETUP}" = "1" ]; then
        >&2 echo "the top mass analysis was already succesfully setup"
    ...
    #
    # global variables
    # (TM = top mass, CF = columnflow)
    #

    # start exporting variables
    export TM_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${TM_BASE}"
    export CF_REPO_BASE_ALIAS="TM_BASE"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$TM_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "topmass_store"
            ...
            query TM_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${TM_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${TM_BASE}/bin:${PATH}"
    export PYTHONPATH="${TM_BASE}:${TM_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${TM_BASE}/.git" ]; then
        local m
...
main() {
    ...
    if setup_topmass "$@"; then
        cf_color green "top mass analysis successfully setup"
        return "0"
    else
        local code="$?"
