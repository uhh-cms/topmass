...
setup_aj() {
    ...
    #   - "AJ_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "AJ_" variables is documented below.
    #
    ...
    if [ "${AJ_SETUP}" = "1" ]; then
        >&2 echo "the topmass_alljets analysis was already succesfully setup"
    ...
    #
    # global variables
    # (AJ = topmass_alljets, CF = columnflow)
    #

    # start exporting variables
    export AJ_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${AJ_BASE}"
    export CF_REPO_BASE_ALIAS="AJ_BASE"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$AJ_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "aj_store"
            query CF_STORE_LOCAL "Default local output store" "\$CF_DATA/\$CF_STORE_NAME"
            query CF_WLCG_CACHE_ROOT "Local directory for caching remote files" "\$CF_DATA/aj_cache"
            ...
            query AJ_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${AJ_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${AJ_BASE}/bin:${PATH}"
    export PYTHONPATH="${AJ_BASE}:${AJ_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${AJ_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${AJ_BASE}/modules" ); do
            cf_init_submodule "${AJ_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${AJ_BASE}/.law"
    export LAW_CONFIG_FILE="${AJ_BASE}/law.cfg"
...
main() {
    ...
    if setup_aj "$@"; then
        cf_color green "topmass_alljets analysis successfully setup"
        return "0"
    else
        local code="$?"
