...
```
law run cf.CreateDatacards --inference-model default_1D --hist-hooks qcd   --version v1_Analysis  --configs 2017_v9  --selector default --cf.MergeHistograms-workflow htcondor --cf.MergeHistograms-pilot --htcondor-memory 1700MB --htcondor-runtime 1h --workers 1000 --tasks-per-job 20 
```

To create the 2D data card:
```
 law run cf.CreateDatacards --inference-model default_2D --hist-hooks qcd,unrolling_2D   --version v1_Analysis  --configs 2017_v9  --selector default --cf.MergeHistograms-workflow htcondor --cf.MergeHistograms-pilot --htcondor-memory 1700MB --htcondor-runtime 1h --workers 1000 --tasks-per-job 20
```
...
The produced datacard and shapes files can be copied and further processed using the combine tool in the corresponding repository:
https://gitlab.cern.ch/cms-analysis/top/massrun2aj/datacards
